...
// Seuils critiques
const CRITICAL_CHECKS = ['supabase', 'railway', 'aminyo_api'];
const WARNING_CHECKS = ['providers'];
...
async function main() {
  ...
  log('  API Express Railway (watchdog)...');
  results.aminyo_api = runCheck('check-aminyo-api.js');
  if (results.aminyo_api.status === 'FAIL') {
    criticalFound = true;
    log('  ❌ API Express CRITIQUE');
  } else log('  ✅ API Express OK');
  
  // Checks warning
  log('  Make.com... ignoré');
  results.make = { status: 'OK', message: 'Remplacé par API Express' };
  ...
  if (criticalFound || warningFound) {
    ...
    if (now - lastAlert > 3600000) {
      ...
      failedEntries.forEach(function([key, res]) {
        ...
        } else if (key === 'aminyo_api') {
          alertLines.push('❌ API Express Aminyo — endpoint health injoignable');
          actionRecommandee = 'Vérifier le déploiement Railway : aminyo-os-api-production.up.railway.app/health';
